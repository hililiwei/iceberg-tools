...
public class Main {
    ...
    Main() {
        tools = new TreeMap<>();
        for (Tool tool : new Tool[] {new ManifestFileReadTool()}) {
    ...
    private int run(String[] args) throws Exception {
        ...
        try (InputStream versionInput =
                Main.class.getClassLoader().getResourceAsStream("VERSION.txt")) {
            printStream(versionInput);
        }
        System.err.print(" of ");
        try (InputStream noticeInput =
                Main.class.getClassLoader().getResourceAsStream("META-INF/NOTICE")) {
    ...
    private static void printStream(InputStream in) throws Exception {
        ...
        for (int i = in.read(buffer); i != -1; i = in.read(buffer)) System.err.write(buffer, 0, i);
    }
